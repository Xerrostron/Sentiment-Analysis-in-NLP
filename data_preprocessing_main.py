...
def _read_data():#test
	...
	##
 directory_path = r'C:\Users\Dimen\OneDrive\Documents\Programming\PythonPrograms\Assignment4DataMining'    
 train_file = 'training_raw_data.csv'
 test_file = 'test_raw_data.csv'
...
def _data_preprocessing(input_data):#Test
...
def _convert_labels(input_data):#test
 ...
 input_data['Label'] = input_data['Label'].map(label_map)
...
def _map_tokens2index(input_data,top_K = 500):
    ...
    #seq_words column: contains the integer length of a review
    tokens_list = []
    for l in words:
        tokens_list.extend(l)
    #make an element in tokens_list the size of a seq_words for each review
